...
用例基于 Node 内置 test runner，不引入额外框架，用 `tests/helpers/wx-mock.js` 替身覆盖登录、令牌轮换、并发刷新、401 重放、退出、日志 API 和文案完整性。当前 46 个用例全部通过。这些用例不校验 WXML/WXSS 渲染，界面仍需在开发者工具和真机验证。
...
- 日志列表、压缩图片发布和点赞已接入；日志详情、编辑删除及评论回复界面尚未同步，注意、倒计时和站内通知仍为占位页。
- Markdown 当前以纯文本安全展示；Mermaid、LaTeX 和完整 Markdown 渲染仍需小程序端独立选型与安全验证。
