...
当前已完成首个认证切片：客户端提供微信/QQ 独立账号登录页、会话恢复、安全令牌存储和退出；后端提供开发环境 Fake Provider、用户创建、访问令牌、刷新令牌轮换与会话撤销。真实微信/QQ SDK 和服务端换票仍需开放平台资质与凭据。内容发布、评论、倒计时和推送等业务接口尚未实现。
...
1. 已完成：仓库分层、客户端导航与主题、本地化、基础设施、账号/家庭模型、微信/QQ 独立账号的内部认证闭环，以及 macOS arm64/iOS 无签名构建验证。
2. 下一步：验证微信/QQ 开放平台资质并替换 Fake Provider，同时实现家庭邀请和成员校验。
3. 后续：日志/注意/倒数领域、图片上传、评论点赞、Outbox、实时同步和系统推送。
