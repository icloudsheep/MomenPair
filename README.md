...
当前已完成认证与家庭空间两个切片：客户端提供微信/QQ 独立账号登录、会话恢复、安全令牌存储，以及家庭创建、邀请码加入、成员管理和退出；后端提供 Fake Provider、令牌轮换、一次性邀请和即时成员权限校验。真实微信/QQ SDK 和服务端换票仍需开放平台资质与凭据。内容发布、评论、倒计时和推送等业务接口尚未实现。
...
1. 已完成：仓库分层、客户端基础框架、微信/QQ 独立账号的内部认证闭环、家庭创建/邀请/成员管理，以及 macOS arm64/iOS 无签名构建验证。
2. 下一步：实现日志与注意内容的发布、家庭内列表、详情、编辑和删除。
3. 后续：倒数、图片上传、评论点赞、Outbox、实时同步和系统推送；开放平台资质就绪后替换 Fake Provider。
